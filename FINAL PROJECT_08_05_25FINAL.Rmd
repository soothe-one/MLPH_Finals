...
### Checking the data distribution by outcome variable
...
train_y <- as.numeric(train_data$poisonous)  # Convert to numeric 0/1

test_x <- model.matrix(poisonous~ ., data = test_data)[,- 1]
test_y <- as.numeric(test_data$poisonous)  # Convert to numeric 0/1
...
# Make predictions
en_pred <- predict(final_model, newx = test_x, type = "response", s = best_lambda)
...
train_x <- model.matrix(poisonous ~ ., data = train_data)[, -1]
train_y <- factor(train_data$poisonous, levels = c("poisonous", "edible"))

test_x <- model.matrix(poisonous ~ ., data = test_data)[, -1]
test_y <- factor(test_data$poisonous, levels = c("poisonous", "edible"))
...
# Select best alpha and lambda based on AUC
best_auc <- -Inf
for (alpha in alpha_values) {
  cv_fit <- results[[as.character(alpha)]]
  max_auc <- max(cv_fit$cvm)
...
# Predictions
train_pred <- predict(final_model, newx = train_x, type = "response")
test_pred <- predict(final_model, newx = test_x, type = "response")

train_class <- ifelse(train_pred > 0.5, "poisonous", "edible") |> factor(levels = c("poisonous", "edible"))
test_class  <- ifelse(test_pred > 0.5, "poisonous", "edible") |> factor(levels = c("poisonous", "edible"))

# Create tibbles for yardstick
train_eval <- tibble(
  truth = train_y,
  .pred_class = train_class,
  .pred = as.vector(train_pred)
)

test_eval <- tibble(
  truth = test_y,
  .pred_class = test_class,
  .pred = as.vector(test_pred)
)

# Metrics function
evaluate_metrics <- function(eval_tbl, label = "Set") {
  metrics <- metric_set(accuracy, sens, spec, f_meas, roc_auc, brier_class)
  result <- metrics(eval_tbl, truth = truth, estimate = .pred_class, .pred = .pred)
  result$set <- label
  return(result)
}

# Apply metrics
train_metrics <- evaluate_metrics(train_eval, "Train")
test_metrics <- evaluate_metrics(test_eval, "Test")
...
# Plot ROC curves
roc_train <- roc(response = train_y, predictor = as.vector(train_pred), levels = c("edible", "poisonous"))
roc_test  <- roc(response = test_y, predictor = as.vector(test_pred), levels = c("edible", "poisonous"))

plot(roc_test, col = "blue", lwd = 2, main = "ROC Curves")
lines(roc_train, col = "green", lwd = 2, lty = 2)
legend("bottomright", legend = c("Test", "Train"), col = c("blue", "green"), lwd = 2, lty = c(1, 2))
...
ggplot(important_vars, aes(x = reorder(variable, coefficient), y = coefficient)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  coord_flip() +
  labs(title = paste("Top 20 Important Variables (Alpha =", best_alpha, ")"),
